...
class Qwen3TTSSpeakerEncoderConfig(PretrainedConfig):
    ...
    It is used to instantiate a Qwen3TTS speaker encoder model according to the specified arguments,
    defining the model architecture.

    The architecture is based on the ECAPA-TDNN model.
    ...
    Args:
        ...
        enc_channels (`list[int]`, *optional*, defaults to `[512, 512, 512, 512, 1536]`):
            ...
            The first channel size is for the initial TDNN layer, the intermediate ones for the
            `SqueezeExcitationRes2NetBlock` layers, and the last one for the multi-layer feature aggregation.
...
class Qwen3TTSTalkerCodePredictorConfig(PretrainedConfig):
    ...
    Args:
        ...
        hidden_size (`int`, *optional*, defaults to 32768):
        ...
        num_key_value_heads (`int`, *optional*, defaults to 32):
            Number of key_value heads used for GQA/MQA/MHA.
        ...
        use_sliding_window (`bool`, *optional*, defaults to `False`):
            Sliding window attention on/off.
        sliding_window (`int`, *optional*, defaults to 32768):
            SWA window size.
        max_window_layers (`int`, *optional*, defaults to 28):
            Full-attention layers before SWA.
...
class Qwen3TTSTalkerConfig(PretrainedConfig):
    ...
    IMPORTANT FIX:
    Some model code expects HuggingFace-standard fields `pad_token_id` / `bos_token_id` / `eos_token_id`.
    Older/minified configs only exposed codec_* ids. We map standard fields to codec ids by default.
    """
    ...
    def __init__(
        ...
        # --- FIX: add HF-standard token ids ---
        pad_token_id=None,
        bos_token_id=None,
        eos_token_id=None,
        **kwargs,
    ):
        # Map HF-standard token ids to codec ids unless explicitly provided.
        if pad_token_id is None:
            pad_token_id = codec_pad_id
        if bos_token_id is None:
            bos_token_id = codec_bos_id
        ...
        # Ensure attributes exist even if some code reads them directly
